...
Production-ready code for interview and portfolio use
...
def generate_funnel_data(n_users=50000, n_days=365):
    """
    Generate realistic e-commerce funnel dataset with:
    - Multiple sessions per user
    - Realistic drop-off patterns
    - Device and traffic source bias
    ...
    for user_id in range(1, n_users + 1):
        # Session frequency: 70% single, 20% 2-3, 10% 4+
        ...
        for session_num in range(n_sessions):
            ...
            # Base conversion probability (varies by device & traffic source)
            ...
            events_list.append({
                ...
                'product_price': np.random.exponential(2000) + 500,
                'session_duration_seconds': 0,
                'conversion_flag': 1 if will_convert else 0
            })
            
            # Define progression probabilities
            progression = {
            ...
            while current_event != 'Purchase' and current_event in progression:
                ...
                # If not converting, reduce progression probability
                if not will_convert:
                ...
                if np.random.rand() < probability:
                    ...
                    events_list.append({
                        ...
                        'traffic_source': traffic_source,
                        'product_category': np.random.choice(categories),
                        'product_price': events_list[-1]['product_price'],
                        'session_duration_seconds': int(session_duration),
                        'conversion_flag': 1 if (will_convert and next_event == 'Purchase') else 0
                    })
                    current_event = next_event
                else:
                    break
        
        if (user_id + 1) % 10000 == 0:
...
def clean_data(df):
    ...
    # Remove bot traffic (>100 events in <5 minutes)
    suspicious = df.groupby('session_id').filter(
        lambda x: len(x) > 100 and 
                  (x['timestamp'].max() - x['timestamp'].min()).total_seconds() < 300
    )
    df = df[~df['session_id'].isin(suspicious['session_id'])]
    print(f"  Removed {len(suspicious)} bot traffic events")
...
def perform_eda(df):
    ...
    for event in event_order:
        ...
        pct = 100 * count / df[df['event_type'] == 'App Open']['user_id'].nunique()
...
def create_visualizations(df, user_features):
    ...
    funnel_data = df['event_type'].value_counts().reindex(event_order)
    ...
    axes[0, 1].set_ylim(0, max(device_conv.values) * 1.2)
    ...
    # 4. Drop-off Analysis
    drop_off_data = []
    prev_count = funnel_data.values[0]
    for stage, count in zip(funnel_data.index, funnel_data.values):
        drop_off = ((prev_count - count) / prev_count * 100) if prev_count > 0 else 0
        drop_off_data.append(drop_off)
        prev_count = count
    
    axes[1, 0].bar(range(len(drop_off_data)-1), drop_off_data[:-1], color='#FF6B6B')
    axes[1, 0].set_xticks(range(len(drop_off_data)-1))
    axes[1, 0].set_xticklabels([f'{i}→{i+1}' for i in range(len(drop_off_data)-1)], rotation=45)
    ...
    top_regions['conv_rate'] = (
        top_regions['sum'] / top_regions['count'] * 100
    ).sort_values(ascending=False).head(10)
    
    axes[1, 2].barh(range(len(top_regions['conv_rate'])), 
                    top_regions['conv_rate'].values, color='#45B7D1')
    axes[1, 2].set_yticks(range(len(top_regions['conv_rate'])))
    axes[1, 2].set_yticklabels(top_regions['conv_rate'].index)
    ...
    # Heatmap: Device vs Traffic Source
    # Heatmap: Device vs Traffic Source
    ...
    ax = sns.heatmap(
        heatmap_data, # ✅ Use heatmap_data (not df_pivot)
        annot=True, 
        fmt='.2f', 
        cmap='viridis', 
        cbar_kws={'label': 'Conversion Rate (%)'}
    )
    plt.title('Conversion Rate Heatmap: Device vs Traffic Source')
...
def build_ml_models(user_features):
    ...
    return {
        ...
        'scaler': scaler, 'X_test': X_test_scaled, 'y_test': y_test,
        'y_pred_proba_lr': y_pred_proba_lr, 'y_pred_proba_rf': y_pred_proba_rf,
...
def generate_insights(df):
    ...
    dropoff = ((product_count - cart_count) / product_count * 100)
    ...
    print(f"\n   Recommendations:")
    print(f"   • Add customer reviews and ratings")
    print(f"   • Improve product images (add 360° view)")
    print(f"   • Display competitor comparison")
    print(f"   • Show delivery timeline clearly")
    
    # Insight 2: Device Performance
    print("\n2. 📱 DEVICE DISPARITY: Mobile Conversion Gap")
    device_conv = df.groupby('device_type')['conversion_flag'].mean() * 100
    desktop_conv = device_conv['Desktop']
    mobile_conv = device_conv['Mobile']
    gap = desktop_conv - mobile_conv
    print(f"   Desktop: {desktop_conv:.2f}% | Mobile: {mobile_conv:.2f}% (Gap: {gap:.2f}%)")
    print(f"   Mobile Traffic %: {(df[df['device_type']=='Mobile']['user_id'].nunique() / df['user_id'].nunique() * 100):.1f}%")
    print(f"   Revenue Opportunity: ${(gap/100 * 220000 * 2500 * 12 / 1_000_000):.0f}M annually")
    print(f"\n   Recommendations:")
    print(f"   • Optimize mobile checkout form (reduce fields to 3)")
    print(f"   • Implement mobile payment options (Apple/Google Pay)")
    print(f"   • Improve page load speed (<2 seconds target)")
    ...
    print(f"\n   Recommendations:")
    print(f"   • Increase investment in Organic ({traffic_conv['Organic']:.2f}%)")
    print(f"   • Improve Social campaign targeting ({traffic_conv['Social']:.2f}%)")
    print(f"   • Maintain Paid quality assurance")
    ...
    print(f"\n   Recommendations:")
    print(f"   • Investigate top region success factors")
    print(f"   • Localize payment methods by region")
    print(f"   • Regional marketing investment shift")
    
    # Insight 5: Session Duration Impact
    ...
    ratio = converters_duration / non_converters_duration
    print(f"   Converters Avg: {converters_duration:.0f}s")
    print(f"   Non-converters Avg: {non_converters_duration:.0f}s")
    print(f"   Ratio: {ratio:.1f}X")
    print(f"\n   Recommendations:")
    print(f"   • Improve product content to keep users engaged")
    print(f"   • Add comparison tools and filters")
    print(f"   • Implement product recommendation engine")
...
def main():
    ...
    print(f"  • Total Events: {len(df):,}")
    print("\nNext Steps:")
    print("  1. Export SQL queries from FUNNEL_SQL_QUERIES.sql")
    print("  2. Deploy models to production")
    print("  3. Create dashboards in Power BI/Tableau")
